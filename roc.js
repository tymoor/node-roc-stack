...
// An exception response from a ROC will have
// an OPCODE of 255
const EXCEPTION_OPCODE = 255;
...
RocRequestStack.prototype.request = function(opCode) {
	...
	if (!client.REQUESTS[opCode]) {
		throw new Error('"REQUESTS['+opCode+']" in "client.js" is not implemented!');
	}
	this.opCode = opCode;
		...
		finishRequest(this);
			...
			.then( function (results) {
				...
				if (!client.RESPONSES[ref.responseHeader.opCode]) {
					return ref.emit('error', new Error('"RESPONSES['+ref.responseHeader.opCode+']" in "client.js" is not implemented!'));
				}
				try {
...
function finishRequest(ref) {
	...
	var needMore = client.NEEDMORE[ref.opCode].call(ref, ref.dataBufferList);
...
function readHeader(bufferlist) {
  var header = Binary.parse(bufferlist)
    .word8('toUnit')
    .word8('toGroup')
    .word8('fromUnit')
    .word8('fromGroup')
    ...
    //.end()
	.vars;
  //bufferlist.advance(ROC_HEADER_LENGTH);
  //header.crc = Binary.parse(bufferlist).skip(header.dataLength).word16le('crc').end().vars.crc;
  //console.log(header);
  return header;
}
